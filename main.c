...
int main(void)
{
  ...
  MX_I2C1_Init();
  /* USER CODE BEGIN 2 */
  DS1307_SetTime(12, 30, 00);        // Set Time only once & Clock will Start from this value
  /*
   * Logic To Write Data in EEPROM
  EEPROM_WriteByte(0x0120, 0x55);
  printf("EEPROM Write Done\n");


   * Logic To Read from EEPROM
  uint8_t value = EEPROM_ReadByte(0x0120);
  ...
  */
  ...
  while (1)
  {
	  ...
	  HAL_Delay(1000);
	  /* Logic to find Address of AT24C32 EEPROM

	    printf("Scanning I2C bus...\r\n");
	  for(uint8_t addr = 1; addr < 128; addr++)
	  {
		  //printf("Program inside For loop\r\n");
	      if (HAL_I2C_IsDeviceReady(&hi2c1, (addr << 1), 2, 10) == HAL_OK)// Prints 2 address 1) At24C32 EEPROM - 0X50 (7-Bit) ; 2) DS1307 (RTC) → 0X68 (7-bit)
	      {
